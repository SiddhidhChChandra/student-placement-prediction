...
import os
import sqlite3  # Built-in lightweight database library

# Find the exact path of this folder
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
model_path = os.path.join(BASE_DIR, 'model.pk')
db_path = os.path.join(BASE_DIR, 'placement_history.db') # Database file path
...
def init_db():
    ...
    cursor = conn.cursor()
    # Create a table to store history logs if it doesn't exist yet
    cursor.execute('''
...
# Save user query data into the SQLite log database
...
# Retrieve historical logs to display inside the terminal
...
# Initialize the database table structure on application bootup
init_db()
...
# Professional UI Theming Overrides via CSS
st.markdown("""
    <style>
    .main .block-container { padding-top: 2rem; max-width: 1200px; }
    div[data-testid="stMetricValue"] { font-size: 24px; font-weight: 700; color: #1e293b; }
    div[data-testid="stForm"] { border: 1px solid #e2e8f0; border-radius: 8px; box-shadow: 0 1px 3px 0 rgba(0,0,0,0.05); }
...
with right_column:
    if submit_button:
        ...
        # Calculate status before matching templates so we can log to database cleanly
        final_status_text = ""
        ...
        # COMMIT TO THE DATABASE FILE INSTANTLY ON USER SUBMIT BUTTON CLICK
        ...
        with tab3:
            ...
            st.caption("Showing the absolute latest entries pulled directly out of your SQLite datastore file:")
            
            # Fetch logs directly from our persistent SQLite database file
